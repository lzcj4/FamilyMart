...
namespace DAL.Model
{
    public class FMDBHelper : IDisposable
    {
        ...
        public IEnumerable<GoodsRecord> GetGoodsRecord(string sql)
        {
            using (SQLiteDataReader reader = dbHelper.Query(sql))
            {
                while (reader.Read())
                {
                    ...
                    item.DialyReport = GetDialyReport(string.Format("{0} where Id={1}", sqlSelectDialyReport, report_id)).FirstOrDefault();
                    yield return item;
                }
            }
        }
        ...
        {
            return GetDialyReport(sqlSelectDialyReport + sqlOrderbyAsc);
        }
